...
class PGEntity {
   ...
   *
   * The format for database ids is a JSON object with all the field names of 
   * the id as properites and the field values of the keys as values, JSON.stringify()'d
   * and then base64.
   *
   * TODO: Verify that JSON.stringify is definitely deterministic based on the
   * various versions of node...  Also consider making a special string class
   * called PGEntityIDString which is restricted to a known safe subset of
   * characters to use
   */
  calculateId(value) {

    if (this._id.length === 1 && typeof value === 'string') {
      return Buffer.from(value).toString('base64');
    } else {
      let key = [];

      for (let id of this._id) {
        if (value[id]) {
          key.push(Buffer.from(value[id].toString()).toString('base64'));
        } else {
          throw new PGEntityIDError(`Missing ${id}`);
        }
      }
  ...
  // TODO implement an etag() function which loads the etag for a specific document from an id
  ...
  async update(document, modify) {
    ...
    //
    // TODO: Decide if we want this or if we'd rather have the proxy in the document
    // block changes to the id properties, so that the id is immutable
