...
async function run({args, returnOutput=false}) {
  let options = {};
...
class PGTestDatabase {
  ...
  async runPSQLCommand(command) {
    if (!this.dbproc) {
      throw new Error(`PSQL Commands can only run on started dbs`);
    }
  ...
  async runPSQLFile(file) {
    if (!this.dbproc) {
      throw new Error(`PSQL Files can only run on started dbs`);
    }
